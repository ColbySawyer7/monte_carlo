...
// Shared Monte Carlo configuration state
// This composable allows multiple components to access and modify the same Monte Carlo config
// Based on DESconfigDefaults but with iterations support
...
const INITIAL_DEFAULTS = {
  ...
  simSettings: {
    ...
    iterations: 5000, // Monte Carlo specific
...
// Simulation Settings State - initialized from defaults (includes iterations)
...
export function saveToLocalStorage(scenarioName?: string) {
  try {
    ...
    localStorage.setItem('monteCarloScenarioState', JSON.stringify(state))
  } catch (error) {
...
export function loadFromLocalStorage(): { success: boolean; scenarioName?: string } {
  try {
    const saved = localStorage.getItem('monteCarloScenarioState')
...
export function MonteCarloConfigDefaults() {
  return {
    processTimes,
    simSettings,
    missionTypes,
